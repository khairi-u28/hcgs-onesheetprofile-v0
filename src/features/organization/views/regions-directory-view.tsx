...
export function RegionsDirectoryView() {
  const router = useRouter();
  const employees = usePortalStore((state) => state.employees);
  const organization = getOrganizationBranches();

  const [search, setSearch] = useState("");
  const [sorting, setSorting] = useState<SortingState>([{ id: "regionName", desc: false }]);

  const regionData = useMemo(() => {
    const map = new Map<string, { areas: Set<string>; count: number; sumKpi: number; countKpi: number; sumHav: number; countHav: number }>();

    for (const org of organization) {
      if (!map.has(org.region)) {
        map.set(org.region, { areas: new Set(), count: 0, sumKpi: 0, countKpi: 0, sumHav: 0, countHav: 0 });
      }
      map.get(org.region)!.areas.add(org.area);
    }

    for (const emp of employees) {
      const branchInfo = organization.find(b => b.branchCode.toUpperCase() === emp.branchCode.toUpperCase());
      const region = branchInfo?.region || "Unknown Region";
      
      if (!map.has(region)) {
         map.set(region, { areas: new Set(), count: 0, sumKpi: 0, countKpi: 0, sumHav: 0, countHav: 0 });
      }
    ...
    return Array.from(map.entries()).map(([regionName, stats]) => ({
      regionName,
      ...
      totalAreas: stats.areas.size,
    })).filter(r => r.regionName.toLowerCase().includes(search.toLowerCase()));
  ...
  const columns = useMemo(() => [
    ...
    columnHelper.accessor("totalAreas", {
      header: "Total Areas",
  ...
  return (
    <div className={getMainContentClasses("space-y-6 pb-10")}>
      <PageHero
        ...
        title="Regions"
        description="High-level overview of all regions."
      />
